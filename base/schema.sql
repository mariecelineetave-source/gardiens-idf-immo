-- =====================================================================
-- gardiens.idf.immo — schéma de la base et règles d'accès
--
...
create table if not exists opportunites (
  ...
  gardien_id      uuid not null references gardiens(id) on delete restrict,
  ...
  numero_transmis    boolean not null default false,  -- le gardien lui a donné notre numéro
...
create index if not exists opportunites_gardien    on opportunites (gardien_id, partagee_le desc);
...
create index if not exists primes_gardien on primes (gardien_id, cree_le desc);

-- Cumul annuel versé à un gardien : sert à l'informer avant que la question
...
returns integer language sql stable as $$
  ...
  from primes
  where gardien_id = g and statut = 'versee'
...
language sql stable security definer set search_path = public as $$
  ...
  from opportunites o
  join gardiens g on g.id = o.gardien_id
  where o.cle_bien = cle
  order by o.partagee_le asc;
$$;
...
-- --- gardiens : chacun sa fiche ---
drop policy if exists gardien_lit_sa_fiche on gardiens;
create policy gardien_lit_sa_fiche on gardiens
  for select using (id = auth.uid() or est_admin());

drop policy if exists gardien_cree_sa_fiche on gardiens;
create policy gardien_cree_sa_fiche on gardiens
  for insert with check (id = auth.uid());

drop policy if exists gardien_modifie_sa_fiche on gardiens;
create policy gardien_modifie_sa_fiche on gardiens
...
-- --- opportunités ---
-- Le gardien voit les siennes et peut en créer. Il ne peut ni les modifier
...
create policy gardien_lit_ses_opportunites on opportunites
  for select using (gardien_id = auth.uid() or est_admin());

drop policy if exists gardien_partage on opportunites;
create policy gardien_partage on opportunites
  for insert with check (gardien_id = auth.uid());
...
create policy lecture_journal on evenements
  for select using (
    est_admin() or exists (
      ...
      where o.id = evenements.opportunite_id and o.gardien_id = auth.uid()
    )
  );
...
create policy gardien_lit_ses_primes on primes
  for select using (gardien_id = auth.uid() or est_admin());
...
--
--   b) Vérifier que le verrou tient. Connectée en tant que gardien de test,
--      cette requête doit renvoyer zéro ligne :
--
--        select * from opportunites where gardien_id <> auth.uid();
--
